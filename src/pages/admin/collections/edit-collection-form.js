...
export const EditCollectionForm = ({
  ...
  const selectionModel = [collectionId];
  ...
  const { data: allCollections = [] } = useQuery(["userCollections"], () =>
    getCollections()
  );
  ...
  const filteredAllFiles = allFiles.files.filter(
  ...
  const filteredAllInternalCollections = allCollections.filter(
    ...
    >
      ...
      {isEdit && (
        <Grid>
          <Button
            variant="secondary"
            onClick={setOpenConfirmDeleteCollections}
            sx={{
              display: "flex",
              width: "95%",
              outline: "0px",
